...
if len(sys.argv) <= 1 :
    ...
    if os.path.isfile(sys.argv[1])==True:
        ...
        for i in range(len(l1)):
            ...
            #top  five words
        with open('extraProblemOutput.csv', 'w', encoding='utf-8') as fOut:
            #defining writer using DictWriter
            writer = csv.DictWriter(fOut,fieldnames = ['word','frequency'] )
            #heading
            fOut.write('Top 5 words(According to frequency)\n\n')
            #writing the header
            writer.writeheader()
            #writing the top 5 dictionaries from list to csv file
            writer.writerows(list_final[:5])
